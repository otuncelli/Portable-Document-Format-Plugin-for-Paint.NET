...
internal sealed class PdfiumExporter(Document input, Stream output, PropertyBasedSaveConfigToken token, Surface scratchSurface, ProgressEventHandler progressCallback) 
    : PdfExporter(input, output, token, scratchSurface, progressCallback)
{
    public override void Export(CancellationToken cancellationToken)
    {
        ...
        {
            mfp1 = Utils.CreateMemoryFailPoint(Input.Width, Input.Height);
                ...
                try
                {
                    ...
                    using (Bitmap bitmap = surface.CreateAliasedBitmap(bounds, alpha: true))
                    {
                        ...
                        else
                        {
                            ...
                            if (Mode == ExportMode.Cropped)
                            {
                                using (_ = Utils.CreateMemoryFailPoint(width, height))
                            ...
                            {
                                Debug.Assert(pdfBmp is not null);
                                Utils.BufferCopy(src, pdfBmp.Scan0, length);
