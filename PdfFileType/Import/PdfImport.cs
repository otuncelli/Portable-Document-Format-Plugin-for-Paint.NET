...
internal sealed class PdfImport
{
    ...
    public IReadOnlyList<int> PageIndices { get; set; } = [0];
    ...
    public Document Convert(PdfDocument pdf, Action? progress = null, CancellationToken cancellationToken = default)
    {
            ...
            .Select(index =>
            {
                ...
                Lazy<BitmapLayer> lazy = map.GetOrAdd(index, i => new Lazy<BitmapLayer>(() =>
                {
                    ...
                    try
                    {
                        ...
                        {
                            layer1.Name = page.GetLabel() ?? string.Format(SR.PageNumberFormat, i + 1);
        ...
        {
            document.Dispose();
            throw new WarningException(SR.CanceledUponYourRequest, ex);
        }
        catch
        {
            document.Dispose();
    ...
    private Rectangle GetPlacementRectangle(PdfPage page)
    {
        ...
        scaledSizeF = Utils.PointsToPixels(scaledSizeF);
    ...
    private Surface RenderPage(PdfPage page)
    {
        ...
        using (_ = Utils.CreateMemoryFailPoint(width, height, 2))
        using (PDFiumBitmap rhs = new PDFiumBitmap(width, height, hasAlpha: true))
        {
            ...
            try
            {
                if (BlendColor != Color.Empty && BlendColor != Color.Transparent)
                {
                    ...
                    unsafe
                    {
                        CompositionOps.Normal.Static.UnsafeApply(width, height, (ColorBgra*)dst, stride, (ColorBgra*)src, stride);
                    }
                }
                else
                {
                    Utils.BufferCopy(src, dst, stride * height);
                }
            }
            catch
            {
    ...
    {
        string header = SR.PreviewTextHeader + Environment.NewLine + Environment.NewLine;
        string text = SR.PreviewText;
