...
using PaintDotNet;
using PdfFileTypePlugin.Extensions;
...
internal partial class PdfImportDialog : Form
{
    ...
    public Document Result
    {
        get
        {
            ...
            throw new WarningException(SR.CanceledUponYourRequest, new OperationCanceledException());
        }
    ...
    {
        // Information
        GbInformation.Text = SR.Information;
        LblLblPageCount.Text = SR.PageCount;
        LblLblPassword.Text = SR.Password;
        LblLblPageSize.Text = SR.PageSize;

        // Render options
        GbRenderOptions.Text = SR.RenderOptions;

        // Page(s)
        GbPages.Text = SR.Pages;
        RbAllPages.Text = SR.AllPages;
        RbCustomPages.Text = SR.Custom;
        TxtCustomPages.Text = SR.CustomPagesPlaceHolderText;
        TxtCustomPages.Font = new Font(TxtCustomPages.Font, FontStyle.Italic);
        TxtCustomPages.ForeColor = Color.DimGray;
        LblBlendOnto.Text = SR.BlendOnto;
        LblAlignment.Text = SR.Alignment;


        // Layer settings
        GbLayers.Text = SR.Layers;
        CbRestoreLayerProperties.Text = SR.RestoreLayerProperties;
        CbDescending.Text = SR.Descending;
        CbInvisibleLayers.Text = SR.Invisible;
        LblLayerOpacity.Text = SR.Opacity;
        LblBlendMode.Text = SR.BlendMode;

        // Dimensions
        GbDimensions.Text = SR.Dimensions;
        LblWidth.Text = SR.Width;
        LblHeight.Text = SR.Height;
        LblPx1.Text = SR.Px;
        LblPx2.Text = SR.Px;
        CbKeepAspectRatio.Text = SR.MaintainAspectRatio;

        // Other
        UpdateAvailLabel.Text = string.Empty;
        LnkGitHub.Text = SR.GitHubLink;
        LnkForum.Text = SR.ForumLink;
        LblProgress.Text = SR.Ready;
        BtnReset.Text = SR.Reset;
        BtnOK.Text = SR.OK;
    ...
    private void PopulateControls()
    {
        ...
        LblPassword.Text = hasPassword ? SR.Yes : SR.No;
        NudWidth.Value = maxPageWidth;
        NudHeight.Value = maxPageHeight;
        if (pageSizeVaries)
        {
            LblPageSize.Text = SR.Varies;
    ...
    private void IncrementProgress()
    {
        ...
        {
            value = Math.Clamp(value, ProgressBar1.Minimum, ProgressBar1.Maximum);
    ...
    private void UpdateOtherInput()
    {
        ...
        if (ReferenceEquals(lastModifiedNud, NudWidth))
        {
            ...
            newHeight = Math.Clamp(newHeight, 1, Math.Min(NudHeight.Maximum, int.MaxValue / (NudWidth.Value * 4)));
        ...
        else
        {
            ...
            newWidth = Math.Clamp(newWidth, 1, Math.Min(NudWidth.Maximum, int.MaxValue / (NudHeight.Value * 4)));
    ...
    private void CbBgColor_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        DialogResult dr = this.RunOnUIThread(form => colorDialog.ShowDialog(form));
    ...
    {
        if (TxtCustomPages.Text.Equals(SR.CustomPagesPlaceHolderText))
        {
            TxtCustomPages.Font = new Font(TxtCustomPages.Font, FontStyle.Italic);
            TxtCustomPages.ForeColor = Color.DimGray;
        }
    ...
    {
        if (TxtCustomPages.Text.Equals(SR.CustomPagesPlaceHolderText, StringComparison.Ordinal))
        {
    ...
    private async void BtnOK_Click(object sender, EventArgs e)
    {
        ...
        {
            pageIndices.Clear();
            string text = string.Format(SR.CustomFieldError, SR.Custom);
            this.RunOnUIThread(form => MessageBoxEx.Show(form, text, SR.Error, MessageBoxButtons.OK, MessageBoxIcon.Error));
        }
        ...
        if (RbCustomPages.Checked)
        {
            ...
            if (customPages.Equals(SR.CustomPagesPlaceHolderText, StringComparison.Ordinal) || string.IsNullOrWhiteSpace(customPages))
            {
                OnParseFail();
                return;
            }
        ...
        try
        {
            using (cts = new CancellationTokenSource())
            {
                ...
                Result = await Task.Run(() => context.Convert(pdf, IncrementProgress, ctoken), ctoken).ConfigureAwait(false);
                LastUsedConfig.CopyFromContext(context);
            }
        }
        catch (Exception ex) when (ex is OperationCanceledException)
        {
            reason = new WarningException(SR.CanceledUponYourRequest, ex);
        }
        catch (Exception ex)
        {
            reason = ex;
    ...
    private void BtnCancel_Click(object sender, EventArgs e)
    {
        ...
        {
            reason = new WarningException(SR.CanceledUponYourRequest, new OperationCanceledException());
            Close();
        }
    }
    ...
    private static void DrawEnumString<T>(object sender, DrawItemEventArgs e) where T : Enum
    {
        ...
        if (e.Index > -1)
        {
            ...
            using (var solidBrush = new SolidBrush(e.ForeColor))
            {
                ...
                string text = SR.ResourceManager.GetString(resourceKey) ?? SplitCamelCase(value);
